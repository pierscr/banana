...
define([
    ...
    var module = angular.module('kibana.panels.scatterplotFacet', []);
    app.useModule(module);

    module.controller('scatterplotFacet', function ($scope, $timeout, timer, dashboard, querySrv, filterSrv) {
        $scope.panelMeta = {
            modals: [{
                description: "Inspect",
                icon: "icon-info-sign",
        ...
        $scope.get_data = function () {
            ...
            results.then(function (results) {
                ...
                $scope.data = results.facets.categories.buckets.map(function(d){
                   var value = {};
                   var totalInt = [];
                   if($scope.panel.colorField)
                    value[$scope.panel.colorField]=d.val;
                   d.categories.buckets.forEach(function(item){
                     if(
                        item.val.indexOf($scope.panel.xaxis)!== -1 ||
                        item.val.indexOf($scope.panel.yaxis)!== -1 ||
                        item.val.indexOf($scope.panel.bubbleSizeField) !== -1)
                       value[item.val] = +item.x;
                       totalInt.push(item[$scope.panel.bubbleSizeField]);
                   })
                   value[$scope.panel.bubbleSizeField]=totalInt.reduce(function(tot,curr){return curr+tot})/totalInt.length;
                   return value;
                })
    ...
    module.directive('scatterplotFacet', function (dashboard, filterSrv,filterDialogSrv) {
        return {
            restrict: 'E',
