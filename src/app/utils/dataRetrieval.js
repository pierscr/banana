        ...
        //         }]);
          ...
          function createRequest(){
            ...
            function getNodes(nodeList,deactiveGlobalFilter,callback){
              ...
              var nodeFilter="&wt=json&facet=true&facet.pivot=cluster_h&q=*:*&rows=0&facet.limit="+ $scope.panel.max_rows;
              var separator=function(index){
                return 0===index?"\"":" OR \"";
              };
              if(nodeList!==undefined){
                filters+="&fq=cluster_h:"+nodeList.reduce(function(tot,curr,index, arr){return tot+separator(index)+callback(curr)+"\"";},"(")+")";
              }
            ...
            function getGridStep(nodeList){
              ...
              var join="&q={!join from=cluster_h to=Cluster2 fromIndex="+$scope.panel.nodesCore+"}*:*"
              ...
              $scope.sjs.Request()
                  ...
                  .then(function(results){
                      ...
                      .then(function(results){
                          ...
                          stepResults.stepNodes=results.facet_counts.facet_pivot['cluster_h'];
                      ...
                      .then(function(results){
                          //$scope.myGrid.addNode(results.facet_counts.facet_pivot['cluster_h'].map(function(item){ item.year=range.getRange(0);return item;}));
                          stepResults.selfNode=results.facet_counts.facet_pivot['cluster_h'];
